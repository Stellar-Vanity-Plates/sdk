...
type Entry = {
  function_v0?: { name: string; inputs: Field[]; outputs: AbiType[] };
...
export function abiFixtures(spec: Spec) {
  ...
  return entries.flatMap((entry) => {
    ...
    if (!fn || fn.name === "__constructor") return [];
    return [{
      name: fn.name,
      args: Object.fromEntries(
        fn.inputs.map((field) => [field.name, value(field.type)]),
      ),
      result: fn.outputs.length ? value(fn.outputs[0]) : undefined,
    }];
  });
}
